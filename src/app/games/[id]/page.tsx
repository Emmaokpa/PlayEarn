...
import { Award, Smartphone, Loader2, Expand, Minimize } from 'lucide-react';
...
export default function GamePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <AppLayout title={game.name}>
        <div className={cn("h-full w-full flex flex-col space-y-4", isPortraitFullscreen && "fixed inset-0 bg-background z-[100] p-0 m-0")}>
            <Alert className={cn("md:hidden bg-secondary border-secondary-foreground/20 landscape:hidden", isPortraitFullscreen && "hidden")}>
            ...
            <Card className={cn("w-full overflow-hidden bg-black shadow-lg flex-1 min-h-0 relative", "landscape:fixed landscape:inset-0 landscape:z-[100] landscape:rounded-none landscape:border-0", isPortraitFullscreen && "rounded-none border-0")}>
                <iframe
                    src={game.iframeUrl}
                    title={game.name}
                ...
                />
                <div className="absolute top-2 right-2 md:hidden landscape:hidden">
                    <Button
                        variant="secondary"
                        size="icon"
                        onClick={() => setIsPortraitFullscreen(!isPortraitFullscreen)}
                        className="bg-black/50 text-white hover:bg-black/70"
                    >
                        {isPortraitFullscreen ? <Minimize className="h-5 w-5" /> : <Expand className="h-5 w-5" />}
                    </Button>
                </div>
            </Card>
            <div className={cn("flex items-center justify-between", "landscape:hidden", isPortraitFullscreen && "hidden")}>
